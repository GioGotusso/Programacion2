...
begin
  if ( a <>nil) and (a^.dato <> x) then begin
    if ( a^.dato > x) then
...
procedure Pbuscar (a: arbol; x:integer; var g: arbol);
begin
  while ( a <>nil) and (a^.dato <> x) do begin
    if ( a^.dato > x) then
      a:= a^.HI
    else
      a:= a^.HD;
...
begin
  ...
  if (a<> nil) then begin
    ...
    if (g= nil) then
      ...
      write( 'se encontro el numero ' ,g^.dato);
  end;
  readln(x);
end.
