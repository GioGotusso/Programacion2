...
begin
  with(a)do begin
    ...
    readln(nombre);
    if(nombre <> FIN) and (dni<>-1)then begin
...
procedure BusquedaApe(a: arbol; ape:string[50]);
begin
  if(a <> nil)then begin
    busquedaApe(a^.hi,ape);
    if(a^.dato.apellido=ape) then begin
        writeln(a^.dato.apellido);
        writeln(a^.dato.dni);
        writeln(a^.dato.nombre);
    end;
    bsuquedaApe(a^.hd,ape);
  end;
end;
var
  arbolCurso: arbol;
